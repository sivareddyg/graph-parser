...
public class RunGraphToQueryTrainingMain extends AbstractCli {
  ...
  public void initializeOptions(OptionParser parser) {
    ...
    ccgLexicon = parser.accepts("ccgLexicon", "ccg special categories lexicon").withRequiredArg()
        .ofType(String.class).defaultsTo("./data/lexicon_specialCases.txt");
    ccgLexiconQuestions = parser
        .accepts("ccgLexiconQuestions", "ccg special categories Questions lexicon")
    ...
    trainingCorpora = parser
        .accepts("trainingCorpora", "all unannotated training corpora separated by ;")
    ...
    lexicon = parser.accepts("lexicon", "lexicon containing nl to grounded mappings")
        .withRequiredArg().ofType(String.class).required();

    cachedKB = parser.accepts("cachedKB", "cached version of KB").withRequiredArg()
        .ofType(String.class).required();

    testFile = parser.accepts("testFile", "test file containinig questions").withRequiredArg()
        .ofType(String.class).defaultsTo("");

    devFile = parser.accepts("devFile", "development file containinig questions").withRequiredArg()
        .ofType(String.class).defaultsTo("");

    nthreads = parser.accepts("nthreads", "number of threads: >10 preferred").withRequiredArg()
        .ofType(Integer.class).required();
    trainingSampleSize = parser
        .accepts("trainingSampleSize", "number of training samples used in each iteration")
        .withRequiredArg().ofType(Integer.class).required();
    iterations = parser.accepts("iterations", "number of training iterations").withRequiredArg()
        .ofType(Integer.class).required();

    nBestTrainSyntacticParses = parser
        .accepts("nBestTrainSyntacticParses", "number of syntactic parses to use while training")
        .withRequiredArg().ofType(Integer.class).defaultsTo(1);
    nBestTestSyntacticParses = parser
        .accepts("nBestTestSyntacticParses", "number of syntactic parses to use while testing")
        .withRequiredArg().ofType(Integer.class).defaultsTo(1);
    nbestGraphs = parser.accepts("nbestGraphs", "beam size").withRequiredArg().ofType(Integer.class)
        .defaultsTo(100);
    nbestEdges = parser
        .accepts("nbestEdges", "number of edges/types for each ungrounded edge/types")
        .withRequiredArg().ofType(Integer.class).defaultsTo(20);

    // Set these true, or else graph construction mechanism will be
    // completely driven by lexicon
    useSchema = parser.accepts("useSchema",
        "use schema to drive graph construction - use this flag, otherwise search space explodes")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    useKB = parser
        .accepts("useKB", "use KB to drive graph construction, reduces search space drastically")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    groundFreeVariables = parser.accepts("groundFreeVariables",
        "Ground free variables which do not have any entity clue e.g. ground city(x) where x is not known").withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    useEmptyTypes = parser.accepts("useEmptyTypes", "use type.empty for empty types")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    ignoreTypes = parser
        .accepts("ignoreTypes", "ignore types: subsumes groundFreeVariables and useEmptyTypes")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);

    // Alignment Features
    urelGrelFlag = parser.accepts("urelGrelFlag", "Edge Alignment features - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    urelPartGrelPartFlag = parser
        .accepts("urelPartGrelPartFlag", "subedge alignment features - depends on dataset")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    utypeGtypeFlag = parser.accepts("utypeGtypeFlag", "type alignment features - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    // gtypeGrel imposes strong biases - do not use for cai-yates
    gtypeGrelFlag = parser
        .accepts("gtypeGrelFlag", "edge argument restrictions - depends on dataset")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);

    // Contextual Features
    wordGrelPartFlag = parser
        .accepts("wordGrelPartFlag", "event word and edge feature - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    wordGrelFlag = parser.accepts("wordGrelFlag", "event word and edge feature - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    wordBigramGrelPartFlag = parser
        .accepts("wordBigramGrelPartFlag", "contextual features - a good feature").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(false);
    argGrelPartFlag = parser
        .accepts("argGrelPartFlag", "argument word and edge feature - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    argGrelFlag = parser.accepts("argGrelFlag", "argument word and edge feature - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);

    // Stem features
    stemMatchingFlag = parser
        .accepts("stemMatchingFlag", "edge stem matching feature - a good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    mediatorStemGrelPartMatchingFlag = parser.accepts("mediatorStemGrelPartMatchingFlag",
        "subedge stem matching feature - a good feature").withRequiredArg().ofType(Boolean.class)
        .defaultsTo(true);
    argumentStemMatchingFlag = parser.accepts("argumentStemMatchingFlag",
        "argument word and edge stem matching feature - a good feature").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(true);
    argumentStemGrelPartMatchingFlag = parser.accepts("argumentStemGrelPartMatchingFlag",
        "argument word and subedge stem matching feature - a good feature").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(true);

    // Graph features
    graphIsConnectedFlag = parser
        .accepts("graphIsConnectedFlag", "graph is connected feature - imposes strong constraints")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    // edgeNodeCountFlag entails graphHasEdgeFlag
    graphHasEdgeFlag = parser.accepts("graphHasEdgeFlag", "graph has an edge feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    countNodesFlag = parser
        .accepts("countNodesFlag", "number of nodes in the graph - imposes strong constraints")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);
    // edgeNodeCountFlag and duplicateEdgesFlag are important on cai yates
    edgeNodeCountFlag = parser
        .accepts("edgeNodeCountFlag", "number of connected nodes - depends on dataset")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    duplicateEdgesFlag = parser
        .accepts("duplicateEdgesFlag", "remove duplicate edges next to each other - good feature")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(true);
    // surprisingly grelGrel not useful
    grelGrelFlag = parser
        .accepts("grelGrelFlag", "edge-edge collocation feature - depends on dataset")
        .withRequiredArg().ofType(Boolean.class).defaultsTo(false);

    // Default weights
    useLexiconWeightsRel = parser.accepts("useLexiconWeightsRel",
        "use noisy lexicon to initialise edge weights - a good feature").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(true);
    useLexiconWeightsType = parser.accepts("useLexiconWeightsType",
        "use noisy lexicon to initialise type weights - depends on dataset").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(false);

    initialEdgeWeight = parser
        .accepts("initialEdgeWeight", "set to a positive value if useLexiconWeightsRel is set")
        .withRequiredArg().ofType(Double.class).defaultsTo(1.0);
    initialTypeWeight = parser
        .accepts("initialTypeWeight", "set to a positive value if useLexiconWeightsType is set")
        .withRequiredArg().ofType(Double.class).defaultsTo(-2.0);
    initialWordWeight = parser
        .accepts("initialWordWeight", "set to a negative value to avoid overgenerating edges")
        .withRequiredArg().ofType(Double.class).defaultsTo(-1.0);
    stemFeaturesWeight = parser.accepts("stemFeaturesWeight", "initial weight of stem features.")
        .withRequiredArg().ofType(Double.class).defaultsTo(0.0);

    // Denotation feature
    validQueryFlag = parser.accepts("validQueryFlag",
        "use denotation as feature to see if the graph is valid - good but slow").withRequiredArg()
        .ofType(Boolean.class).defaultsTo(false);
  ...
  public void run(OptionSet options) {
    ...
    try {
      ...
      KnowledgeBase kb = new KnowledgeBase(options.valueOf(cachedKB), relationTypesFileName);

      RdfGraphTools rdfGraphTools = new RdfGraphTools(
          String.format("jdbc:virtuoso://%s:1111", options.valueOf(endpoint)),
          String.format("http://%s:8890/sparql", options.valueOf(endpoint)), "dba", "dba", 4);
      List<String> kbGraphUri = Lists.newArrayList(Splitter.on(";").split(options.valueOf(domain)));

      CcgAutoLexicon normalCcgAutoLexicon = new CcgAutoLexicon("./data/candc_markedup.modified",
          "./data/unary_rules.txt", "./data/binary_rules.txt", options.valueOf(ccgLexicon));
      ...
      GroundedLexicon groundedLexicon = new GroundedLexicon(options.valueOf(lexicon));
      ...
      int nBestTrainSyntacticParsesVal = options.valueOf(nBestTrainSyntacticParses);
      int nBestTestSyntacticParsesVal = options.valueOf(nBestTestSyntacticParses);
      ...
      boolean wordBigramGrelPartFlagVal = options.valueOf(wordBigramGrelPartFlag);
      ...
      boolean argumentStemMatchingFlagVal = options.valueOf(argumentStemMatchingFlag);
