...
public class SemanticCategoryTest {
  ...
  public void testGenerateExpression() {
    ...
    SemanticCategory semCat =
        SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.TYPE);
    assertEquals(semCat.toString(), "(lambda $X:0 (TYPE $X:0 $X:0))");
    // System.out.println(semCat);

    synCat = SyntacticCategory.fromString("S");
    semCat = SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.TYPE);
    ...
    semCat = SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.TYPEMOD);
    assertEquals(semCat.toString(),
        "(lambda $f0 $Y:6 (exists $_:5 (and ($f0 $Y:6) (TYPEMOD $Y:6 $_:5 1 $Y:6))))");

    // EVENT
    // syntactic category for told
    synCat = SyntacticCategory.fromString(
        "(((S[dcl]{_}\\NP{Y}){_}/(S[to]{Z}\\NP{W}){Z}){_}/NP{W}){_};_ 3 W,_ 2 Z,_ 1 Y");
    semCat = SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.EVENT);
    assertEquals(semCat.toString(),
    ...
    synCat = SyntacticCategory.fromString("((S[X]{Y}\\NP{Z}){Y}\\(S[X]{Y}\\NP{Z}){Y}){_};_ 1 Y");
    semCat = SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.EVENTMOD);
    assertEquals(semCat.toString(),
    ...
    synCat =
        SyntacticCategory.fromString("((NP{Y}\\NP{Y}){_}/(S[dcl]{Z}\\NP{Y}){Z}){_};_ 2 Z,_ 1 Y");
    semCat = SemanticCategory.generateSemanticCategory(synCat, SemanticCategoryType.CLOSED);
    assertEquals(semCat.toString(),
  ...
  public void testGetDeepExpression() {
    ...
    SyntacticCategory synCat =
        SyntacticCategory.fromString("((S[X]{Y}\\NP{Z}){Y}\\(S[X]{Y}\\NP{Z}){Y}){W}");
