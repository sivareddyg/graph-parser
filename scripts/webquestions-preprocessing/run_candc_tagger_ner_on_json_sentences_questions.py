...
def parse_sentences(model, first_word_lower=False, add_poss=True, add_prep_dates=True):
    pos_command = """./tools/candc_nbest/bin/pos --model %s | ./tools/candc_nbest/bin/ner --model %s """ % (model + "/pos", model + "/ner")
        ...
        while actual_line != "":
            ...
            for word in parts['words']:
                if word['word'] == '' or re.match("[\s]", word['word']):
                    word['word'] = '?????'
                elif word['word'] == "how-many" or word['word'] == "how-much":
                    # changing how-many and how-much to which
                    word['actual'] = word['word']
                    word['word'] = "which"
                elif word['word'] == "How-many" or word['word'] == "How-much":
                    word['actual'] = word['word']
                    word['word'] = "Which"
            ...
            actualFile.write(json.dumps(parts) + '\n')
        ...
        while inp != "" and line != "":
            ...
            if add_poss:
                ...
                for i, word_pos in enumerate(word_pos_list):
                    (word, pos, ner) = word_pos.split("|")
                    parts['words'][i]['pos'] = pos
                    parts['words'][i]['candcner'] = ner
                ...
                for position, increment in enumerate(insert_positions):
                    ...
                    parts['words'].insert(position + increment, {"word": "'s", "ner": "0", "candcner": "0", "pos": "POS"})  
            ...
            if add_prep_dates:
                prev_pos = "IN"
                insert_positions = []
                word_pos = [(word['word'], word['pos']) for word in parts['words']]
                for i, (word, pos) in enumerate(word_pos):
                    # parts['words'][i]['pos'] = pos
                    # print prev_pos
                    if (prev_pos != "IN") and pos == "CD" and re.match("[0-9]{3,4}$", word) != None:
                        insert_positions.append(i)
                    prev_pos = pos
                for position, increment in enumerate(insert_positions):
                    for i, entity in enumerate(parts['entities']):
                        if entity['index'] >= position + increment:
                            entity['index'] = entity['index'] + 1
                    parts['words'].insert(position + increment, {"word": "in", "ner": "0", "candcner": "0", "pos": "IN"})  
                    # word_pos_list.insert(position + increment, "'s|POS")
