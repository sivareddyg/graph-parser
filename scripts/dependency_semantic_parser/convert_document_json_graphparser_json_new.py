...
measure_types = {0: "type.int", 1: "type.float",
                 3: "type.datetime", 4: "type.datetime"}
...
def document_to_graphparser_input(document):
        ...
        for entity in document['entity']:
            if 'mention' in entity:
                for mention in entity['mention']:
                    # mention is a named entity
                    if 'type' not in mention or mention['type'] == 0:
                        head = mention['phrase']['head']
                        head_to_phrase[head] = ' '.join(
                            sent_words[mention['phrase']['start']: mention['phrase']['end'] + 1])
                        for token_index in range(mention['phrase']['start'], mention['phrase']['end'] + 1):
                            if token_index != head:
                                token_to_mention_or_measure_head[
                                    token_index] = head

                        if 'profile' in entity:
                            profile = entity['profile']
                            if 'mid' in profile:
                                head_to_freebase_id[head] = profile[
                                    'mid'].strip("/").replace("/", ".")

    if 'measure' in document:
        for measure in document['measure']:
            # If there is a head, use it, else treat the first word in the
            # phrase as head.
            head = measure['phrase'][
                'head'] if 'head' in measure else measure['phrase']['start']
            head_to_phrase[head] = ' '.join(
                sent_words[measure['phrase']['start']: measure['phrase']['end'] + 1])
            ...
            if 'type' in measure:
                ...
                head_to_freebase_id[head] = measure_types.get(
                    measure_type, 'type.float')
    ...
    for token_index, token in enumerate(document['token']):
        ...
        for key in token:
            ...
            if key == u"tag":
                ...
                word["head"] = old_token_index_to_new[token[key]]
            elif key == u'label':
                word["dep"] = token[key]
            else:
                word[key] = token[key]
    ...
    if '[nlp_saft.GraphParserLambda.graph_parser_lambda]' in document:
        lambdas = document['[nlp_saft.GraphParserLambda.graph_parser_lambda]']
        if 'dependency_lambda' in lambdas:
            ...
            for dependency_lambda in dependency_lambdas:
                new_lambda = set()
                process_lambda(
                    words, head_to_freebase_id, old_token_index_to_new, dependency_lambda, new_lambda)
        ...
        if 'ccg_lambda' in lambdas:
            ...
            for ccg_lambda in ccg_lambdas:
                new_lambda = set()
                process_lambda(
                    words, head_to_freebase_id, old_token_index_to_new, ccg_lambda, new_lambda)
...
def process_lambda(words, head_to_freebase_id, old_token_index_to_new, old_lambda, new_lambda):
    if not isinstance(new_lambda, set):
        return
    ...
    for basic_expression in basic_expressions:
        ...
        if m:
            predicate = m.group(1)
            arg1 = get_var_to_word_index(
                words, old_token_index_to_new, m.group(2))
            arg2 = get_var_to_word_index(
                words, old_token_index_to_new, m.group(3))
            ...
            if arg2 in head_to_freebase_id:
                arg2 = str(
                    old_token_index_to_new[arg2]) + ":" + head_to_freebase_id[int(arg2)]
            elif type(arg2) == int:
                arg2 = str(old_token_index_to_new[arg2]) + ":x"
            else:
                arg2 = arg2 + ":x"
            if predicate.lower() not in ['what']:
                new_lambda.add("%s(%s , %s)" %
                               (predicate.lower(), str(arg1), str(arg2)))
            continue
        ...
        if m:
            ...
            if arg1 in head_to_freebase_id:
                arg1 = str(
                    old_token_index_to_new[arg1]) + ":" + head_to_freebase_id[int(arg1)]
            elif type(arg1) == int:
